...
#> NAMESPACE:VERSION/signals/denied_dimensions
#
# @within			#NAMESPACE:v1/signals/denied_dimensions
# @executed			as & at the player who triggered the event
# @input score		#authorized NAMESPACE.data equals to 1 or 0 if another datapack has already denied generation
#
# @english_WARNING
...
# execute if score #authorized NAMESPACE.data matches 1 if predicate simplenergy:in_overworld run scoreboard players set #authorized NAMESPACE.data 0

## Another example:
# execute if score #authorized NAMESPACE.data matches 1 if dimension minecraft:overworld run scoreboard players set #authorized NAMESPACE.data 0
